...
les fichiers en cours d'analyse soient dans le dossier courant. On les copie
donc dans le dossier courant. Ça n'est pas très optimal, on pourrait tester avec
des soft-links. 
...
séquence reflète la position sur la séquence non trimmée. 

J'ai modifié polySNP très légèrement, pour qu'il inclut dans sa sortie la
position sur le spectrogramme, qu'il va chercher dans le fichier POLY. C'est
quand même bien plus simple comme ça. Du coup j'ajoute ma version de polySNP à
github, même si j'ignore si j'en ai bien le droit… De toute façon le programme
est sous licence GNU. J'ai fait les modifications nécessaires en accord (si j'ai
bien lu) avec la GPL.

#+BEGIN_SRC R :tangle ./scripts/polymorph.R
  #' ---
  ...
  opts_chunk$set(fig.width=15, fig.height=5, cache = FALSE, dev = 'svg',
                 include = TRUE, echo = TRUE, warning = FALSE, error = FALSE,
                 message = FALSE )
  ...
  #' J'ai voulu regarder sur l'ensemble des séquences, la fréquence des sites avec
  #' un pic secondaire par position. L'idée est de montrer que les pics
  #' secondaires sont plus fréquents aux positions de SNP maqueurs qu'on a
  #' introduit qu'aux autres positions.

  ,#+ freqpicparpos, fig.height = 8
  data %>%
  ...
  #' # Analyse de la répartition des séquences avec pics secondaires dans la plaque
  #'
  ...
  data %>%
    filter(!(plas %in% seqcrade )) %>% # ne garde que les séquences propres 
    ...
    summarise(count = n()) %>%
    ungroup() %>%
    mutate(plas  = as.integer(plas)) %>% # même types pour le join
    ...
    ggtitle("Emplacement des transformants\nmontrant des pics secondaires")

  #' Il semble quand on regarde vaguement comme ça les séquences "contaminées",
  #' _ie_ avec des pics secondaires, qu'elles soient relativement clusterisées
  #' autour de quelques positions. On a donc voulu tester par une simulation si la
  #' répartition est aléatoire ou non. Si elle ne l'est pas, ça peut vouloir
  ...
  #' Il semblerait que les séquences contaminées soient réparties uniformément
  #' dans les séquences _strong_, mais qu'elles soient clusterisées autour de deux
  #' "secteur" dans la plaque _weak_.

  #' On a vu avec Laurent pour mettre au point une simulation. Celle ci nous
  #' permettrait de voir la répartition des puits avec voisins contaminés sous
  #' l'hypothèse d'une répartition aléatoire. Si la moyenne du nombre de puits
  #' avec voisins contaminés n'est obtenue qu'un petit nombre de fois dans la
  #' simulation, alors au risque égal à $10^{-X}$, avec X le nombre de
  #' répétitions, on peut dire que la répartition des puits contaminés n'est pas
  #' dûe au hasard.
  ...
  # une fonction qui initialise la plaque à 0 conta par puit
  initiate_plate <- function() matrix(rep(0, 96), nrow = 8, ncol = 12)

  # une fonction qui transforme l'entrée en matrice. par défaut, crée une plaque
  # vide. doublon. conservée par inertie. et flemme.
  ...
  # génére n plaques contaminées avec n_conta inside.
  ...
  # une fonction qui teste si les puits voisins sont contaminés. Elle est
  # profondément testée. DO NOT EVEN THINK OF TOUCHING IT.
  ...
  # VOISINS sont contaminés
  count_nearby_conta_well <- function(plate) {
    # retrouve les indices des positions par colonne qui sont contaminés
    locate_conta_well <- function(plate) which(plate != 0, arr.ind = TRUE)

    # listes des coordonnées des puits avec conta
    well_list <- locate_conta_well(plate)

    # applique à cette liste la fonction qui permet de retrouver la moyenne du
    # nombre de puit contaminé autour de la position donnée par x[1] et x[2]
    apply(well_list, 1,
  ...
  # compte le nombre de puits contaminés
  count_conta <- function(plate) length(which(plate != 0))

  count_conta(strong_plaque) # => 11
  count_conta(weak_plaque) # => 35

  # nombre de plaque dans la simulation
  n_plaque <- 10000


  #' lance la simulation pour les plaques.

  ## crée une liste de n_plaque qui contient count_conta(*_plaque) réparties
  ## aléatoirement sur la plaque.
  ...
  # une fonction pour convertir les résultats de lapply en vecteur puis data.frame
  # et représenter la distribution.
  ...
  plot_mean_distrib(random_weak_mean) +
    geom_vline(xintercept = count_nearby_conta_well(weak_plaque), color = "red") 

  ,#+ randomstrongmean
  plot_mean_distrib(random_strong_mean) +
    geom_vline(xintercept = count_nearby_conta_well(strong_plaque), color = "red") 

  # en admettant une distribution normale, calcule la statistique associée 
  pnorm(q = count_nearby_conta_well(weak_plaque),
        mean = mean(random_weak_mean %>% unlist()),
        sd   =   sd(random_weak_mean %>% unlist()),
        lower.tail = FALSE) %>% print() # => 0.003313558

  pnorm(q = count_nearby_conta_well(strong_plaque),
        mean = mean(random_strong_mean %>% unlist()),
        sd   =   sd(random_strong_mean %>% unlist()),
        lower.tail = FALSE) %>% print() # => 0.8288228
#+END_SRC

Clique ici pour la version =knitr= du rapport : [[http://sam217pa.github.io/seq-novembre/scripts/polymorph.html][Description des positions
polymorphes]]. 
