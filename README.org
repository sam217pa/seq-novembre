...
En fait j'ai modifié polySNP très légèrement, pour qu'il inclut dans sa sortie
la position sur le spectrogramme, qu'il va chercher dans le fichier poly. C'est
...
#+BEGIN_SRC R
  ...
  data$comt <- plyr::mapvalues(data$comt,
                               ...
                               to = c("D", "B", "A"))
  ## corrige les erreurs de SNP. 
  data$mutt[data$plas == "pS60"] <- "weak"
  data$mutt[data$plas == "pS83"] <- "weak"
  data$mutt[data$plas == "pS92"] <- "weak"
  data$mutt[data$plas == "pS91"] <- "weak"
  data$mutt[data$plas == "pW6" ] <- "strong"
  ...
  ## =============================================================================
  ##
  ## on veut représenter un graphique avec toutes les positions présentant une
  ## base non appelée, où il y aurait du polymorphisme, comparée à la position des
  ## snps.
  ##
  theme_set(
  ...
  ## distribution de la longueur de la trace de conversion.
  data %>% select(plas, len) %>% group_by(plas) %>% summarise(max = max(len)) %>% qplot(data = ., max)  

  ## distribution du nombre de positions polymorphes par transformant
  data %>%
    ...
    summarise(ucnt = n()) %T>% # par plasmide, compte le nombre de positions
                               # polymorphes
    { print( ggplot(data = .) + geom_point(aes(ucnt, plas)) )} %>% 
    filter(ucnt > 80) %T>% # au vu de la distribution, 80 semble un bon cutoff de
                           # propreté
    { print( ggplot(.) + geom_point(aes(ucnt, plas)) )} %>%
    {.$plas} ->
    ...
    function(data, sw = NULL, length = FALSE, clean = TRUE, snp_only = FALSE)
  {
    ...
    find_snppos <-
      function(data) data %>%
                       filter(!is.na(snp)) %>% group_by(refp) %>%
  ...
  plot_second_peak(data, snp_only = FALSE, clean = TRUE, sw = "strong")

  plot_second_peak(data, snp_only = FALSE, clean = TRUE , sw = "weak")
  ...
  data %>%
    ...
    summarise(count = n()) %T>% # détermine la distribution par mutant des ces
                                # switchp
    { print( ggplot(., aes(switchp, count, fill = mutt, color = mutt)) +
             geom_point() + geom_bar(stat = "identity", size = 0.1, alpha = 0.2) 
             ## facet_grid(mutt~.)
            )} %>%
    print()
#+END_SRC
