...
#+BEGIN_SRC sh
  ...
  { ( head -n 1 header-poly.csv ) ; \
    ( tail -n +2 pW85-1073.ab1.poly | sed 's/  /,/g;s/$/,poly/' | awk '$0=1+i++","$0' ) \
  } | cat > pW85.poly.csv
  ...
  # sortie 
  csvjoin -c "position,positionCalledBase" --left pW85.phd.csv pW85.poly.csv | \
      csvjoin -c "SCF.position, refpos" --left - pW85.snp.csv > $filename.csv
...
TODO Rajouter un champ fichier dans tous les fichiers qui permette de savoir
d'où viennent les séquences, autrement que dans le fichier =snp=. 
...
#+BEGIN_SRC R
  ...
  snp <- readr::read_csv("tmp/pW85.snp.csv")
  phd <- readr::read_csv("tmp/pW85.phd.csv")
  poly <- readr::read_csv("tmp/pW85.poly.csv")

  ## left_join(x = poly, y = snp, by = c("area" = "first.area", "type" = "type"))
  ...
  ## snp %>%
  ##   filter(uncalledBase != "N") %>%
  ##   rowwise() %>%
  ##   filter(uncalledBase == second.call) %>%
  ##   group_by(file) %>%
  ##   summarise(count = n()) 
  ### cette approche ne fonctionne pas. les positions SCF ne sont à priori pas les
  ### mêmes, selon qu'on considère le fichier phd ou le fichier snp. ça me fait
  ### chier. beaucoup.
  ...
  snp %>%
    filter(ubas != "N") %>%
    ggplot(data = .,
           aes(x = qpos, y = 1, label = ubas, color = ubas,
               fill = ubas)) +
    geom_text() +
    ## geom_tile(alpha = 0.5) +
    geom_vline( xintercept = snp_pos)


  data <- readr::read_csv("tmp/join.csv") 
