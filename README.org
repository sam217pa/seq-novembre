...
|  A | C | T |   G | majeur | maj_score | mineur | min_score | ratio | position | gene_synt | wt |
|----+---+---+-----+--------+-----------+--------+-----------+-------+----------+-----------+----|
...
#+BEGIN_SRC R :tangle ./scripts/sangerseq.R
  ...
  pairwiseAlignment(pattern = c(primarySeq(pw86),
                                secondarySeq(pw86),
                                primarySeq(pW85),
                                secondarySeq(pW85)),
                    subject = reverseComplement(ref),
                    type = "local-global") ->
    test_pa


  data <- peakAmpMatrix(pW85) %>%
    as.data.frame() %>%
