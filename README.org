...
*** TODO Détermination des SNP calibrés [0/1]
- [ ] créer la fonction =is_a_position= qui détermine si un SNP est bien à la
  position attendu
...
*** TODO Positions terminales de switch [0/3]
...
- [ ] Naïvement on utilise ici la position terminale du dernier SNP. Cependant,
...
*** TODO SNP inattendus [0/1]
- [ ] rajouter le filtre sur les positions calibrées. 
...
#+BEGIN_SRC R :tangle ./scripts/variant_analysis.R
  ...
  ##' @return 
  ##' @author Samuel Barreto
  outlierP <- function(mutant, mutation_type)
  {
  ...
  snp_data %>%
      rowwise() %>%
      mutate(attendu = outlierP(mutant, mutation_type)) %>%
      ungroup() %>%
      ## TODO filter(calibre == "oui") %>% 
      filter(attendu == "non") %>%
...
Écrire une fonction =inside_convP= qui détermine si un SNP est en dehors
